...
This is `dry-run` VMesh packaging of retained BA local evidence and old Intel source intelligence. It is useful because BA can request by `siteId` and receive a laser-fast source package without re-scraping old runs. It is not fresh upstream provider proof, and it does not copy raw BA licensed files, exact coordinates, exact private addresses, signed URLs, or provider order ids into VMesh. The active site is reviewed one at a time; currently `scotland-rural-burmieston` is marked `old_outputs_exhausted` and includes an added 2026-06-03 focused Scotland source-family sweep as `needs_probe` / `needs_license_review` candidate intelligence. All other Europe, Canada, USA, Germany, and Lebanon golden eval sites now also have focused 2026-06-03 packages with site-specific local data and ecosystem context candidates, and every site reports `sourceSweepState: focused_source_sweep_completed`; research-only context is marked for advanced/API mode rather than default BA operational GIS use. Kamloops municipal terrain discovery now uses public City of Kamloops DEM/LiDAR source indexes; exact golden AOI promotion still requires public-safe redacted evidence and Alberta exact AOI live proof still requires operator-local private-coordinate inputs. Operational promotion for any new candidate still requires AOI probes and license checks.

For building footprints, `createBuildingPackageWorkerHandoff` narrows the generic plan into an Overture-first worker request with OSM PBF, Microsoft/Google ML footprint releases, GlobalBuildingAtlas ODbL polygons, and OpenAddresses as reviewed fallbacks. The handoff emits a planned `buildings.json` GeoJSON FeatureCollection contract, source ladder, cache key, and no-synthetic-fill policy. It does not mean vmesh already contains a global building feature index; the output becomes ready only after a worker materializes source-backed footprints for the AOI and writes the artifact plus manifest outside Git or as a sanitized fixture.
