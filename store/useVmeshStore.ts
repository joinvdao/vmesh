...
import { SOURCE_AUTO_DSM_PROVIDER_ID, SOURCE_AUTO_DTM_PROVIDER_ID } from "@/lib/terrainSources";
import { isNorthAmericaTerrainSourceCoordinate } from "@/lib/terrainSourcePreview";
...
const deferredAnalysisLayerIds = new Set([
  "weather",
  "flood",
  "fire",
  "solar",
...
export const useVmeshStore = create<VmeshStore>((set, get) => ({
  ...
  flyToLocation: (location) =>
    set((state) => {
      ...
      const shouldUseOfficialDtmPreview = isNorthAmericaTerrainSourceCoordinate({
        latitude: location.latitude,
        longitude: location.longitude
      });
      const shouldPreserveOfficialDsmPreview =
        shouldUseOfficialDtmPreview &&
        state.selectedTerrainProviderId === SOURCE_AUTO_DSM_PROVIDER_ID;
      const selectedSourcePreviewProviderId = shouldPreserveOfficialDsmPreview
        ? SOURCE_AUTO_DSM_PROVIDER_ID
        : SOURCE_AUTO_DTM_PROVIDER_ID;
      const nextZoom = shouldUseOfficialDtmPreview
        ? Math.max(location.zoom, OSS_MAP_OUTPUT_ZOOM + 0.5)
        : location.zoom;
      ...
      return {
        ...
        activeLayers: {
          ...state.activeLayers,
          imagery: true,
          terrain: shouldUseOfficialDtmPreview ? true : state.activeLayers.terrain
        },
        selectedTerrainProviderId: shouldUseOfficialDtmPreview
          ? selectedSourcePreviewProviderId
          : state.selectedTerrainProviderId,
        ...
        mapStatus: {
          ...state.mapStatus,
          terrain: shouldUseOfficialDtmPreview ? "loading" : state.mapStatus.terrain,
          providerId: shouldUseOfficialDtmPreview
            ? selectedSourcePreviewProviderId
            : state.mapStatus.providerId,
          imagery: "loading",
          message: shouldUseOfficialDtmPreview
            ? `Checking official 1m ${
                shouldPreserveOfficialDsmPreview ? "DSM" : "DTM"
              } source coverage and loading satellite-style imagery`
            : "Loading satellite-style imagery with OSM reference overlay"
        },
  ...
  setSelectedImageryProvider: (providerId) =>
    set((state) => {
      const provider = state.imageryProviders.find((candidate) => candidate.id === providerId);
      return {
        ...
        mapStatus: {
          ...
          message: provider
            ? `Loading ${provider.label} imagery layer`
            : `Loading ${providerId} imagery layer`
        }
      };
    }),
  ...
  setSelectedMacroLayer: (selectedMacroLayer) =>
    set((state) => {
      const sourceLayer = deferredAnalysisLayerIds.has(selectedMacroLayer)
        ? "terrain-elevation"
        : selectedMacroLayer;
      const activeLayers = {
        ...state.activeLayers,
        macro: isH3MacroLayer(sourceLayer),
        terrain: sourceLayer.startsWith("terrain-") ? true : state.activeLayers.terrain,
        imagery: sourceLayer.startsWith("imagery-") ? true : state.activeLayers.imagery
      };

      return {
        selectedMacroLayer: sourceLayer,
        activeLayers,
        visibleHexCount: getVisibleHexCount(
          state.hexDataByTier,
          state.selectedTier,
...
if (typeof window !== "undefined" && process.env.NODE_ENV !== "production") {
  (
    window as Window & {
      __vmeshStore?: typeof useVmeshStore;
    }
  ).__vmeshStore = useVmeshStore;
}
