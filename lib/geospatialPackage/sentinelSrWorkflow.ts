...
function selectPrimaryHandoffArtifact(
  ...
  const handoffArtifacts = artifacts.filter(
    (artifact) =>
      artifact.role === "handoff" &&
      Boolean(artifact.ref) &&
      (artifact.kind === "raster-pmtiles" || artifact.kind === "raster-xyz")
  );
  const readyArtifacts = handoffArtifacts.filter(
    (artifact) => artifact.status === "ready-configured" || artifact.status === "ready-external"
  );

  return (
    readyArtifacts.find((artifact) => artifact.kind === "raster-pmtiles") ??
    readyArtifacts.find((artifact) => artifact.kind === "raster-xyz") ??
    handoffArtifacts.find((artifact) => artifact.kind === "raster-pmtiles") ??
    handoffArtifacts.find((artifact) => artifact.kind === "raster-xyz") ??
    null
  );
}
