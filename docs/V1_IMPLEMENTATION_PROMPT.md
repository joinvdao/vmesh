...
Your task is to build `vmesh` V1: a polished, working, browser-verifiable source-honest geospatial atlas. V1 must show a visible globe on first load and must include production-quality plumbing for an H3 mesh overlay, including U3/U5/U8 mesh tiers. It does not need real provider ingestion or persistent accounts yet.
...
vmesh is a source-honest geospatial atlas. The core interface is a geospatial mesh where each H3 hex can hold:

- Source data: terrain, imagery, open-map context, water, vegetation, infrastructure, parcels, buildings, roads, and land use.
...
- Bottom data overview and selected-hex panels render from the same typed mock data model.
...
- Bottom dashboard: `h-72`, fixed above footer, horizontal source/data overview strip.
- Footer: `h-10`, coordinate readouts, elevation placeholder, H3 tier, H3 resolution, visible hex count, freshness, version/status.

Use `vmesh` branding and `Source Atlas`; do not reuse the screenshot product name.
...
- `getHexagon: d => d.h3Id`
- Fill color uses a stable source-availability tint with a clear selected-cell highlight.
- Selected hex has a clear outline or companion highlight layer.
- Hovered hex shows a tooltip with H3 ID, place name, tier, and source availability.
- `pickable: true`
...
Use simple cards and CSS where appropriate.
