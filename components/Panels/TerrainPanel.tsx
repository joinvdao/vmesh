...
const providerBadges: Record<string, string> = {
  ...
  [SOURCE_AUTO_DTM_PROVIDER_ID]: "Source DTM",
  [SOURCE_AUTO_BEST_DTM_PROVIDER_ID]: "Best source",
  [SOURCE_AUTO_DSM_PROVIDER_ID]: "Source DSM",
...
function ProviderCard({
  ...
  const isRasterDem = toRasterDemSource(provider) !== null;
  const isMapReady = isMapReadyTerrainProvider(provider);
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-[#52616f]">
        <div className="rounded-[7px] bg-[#f7fbfa] px-2 py-1">
          Resolution <span className="font-semibold">{provider.resolution}</span>
        </div>
        <div className="rounded-[7px] bg-[#f7fbfa] px-2 py-1">
      ...
      <div className="mt-2 line-clamp-2 text-[11px] leading-4 text-[#7b8893]">
        {provider.coverage}
      </div>
      ...
      {isSelected ? (
        <div className="mt-3 flex items-center gap-2 text-xs font-semibold text-[#0f766e]">
          ...
          {isRasterDem ? "Feeds terrain and hillshade overlay" : "Displays official source tiles"}
        </div>
      ) : null}
    </button>
  );
...
export function TerrainPanel() {
  ...
  const mapReadyProviders = terrainProviders.filter(isMapReadyTerrainProvider);
  const futureProviders = terrainProviders.filter(
    (provider) => !isMapReadyTerrainProvider(provider)
  );
      ...
      <div className="min-h-0 space-y-3 overflow-y-auto pr-1">
        ...
        <section className="rounded-[10px] border border-[#e3ece9] bg-[#fbfdfc] p-3 text-xs text-[#52616f]">
          ...
          <div className="mt-2 leading-5 text-[#7b8893]">
            ...
            Mapterhorn/Mapzen terrain relief. The selected-cell marker, source layers, imagery, and
            future H3 context remain interactive above it.
          </div>
          <a
