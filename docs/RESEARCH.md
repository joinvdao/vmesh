...
- Benchmark visualizations map cleanly to future vmesh comparison workflows, but climate, hazard, solar, and similar analysis outputs remain deferred from the visible app.
...
- Treat large Earth-system models as deferred analysis providers.
- Convert model outputs into H3-derived local attributes.
- Attach model run metadata, forecast horizon, uncertainty, and provenance to every derived analysis output.
...
- Which climate/weather outputs are reliable enough for future analysis, and which should remain visual/context-only?
...
- What calibration and uncertainty metadata is required before model outputs can influence future analysis?
...
- Biodiversity confidence metadata based on source density and spatial resolution.
...
- Which biodiversity signals should become future analysis inputs, and which should remain explanatory context?
...
- Suggests a compact interchange format for macro summaries, local food-network counts, property signals, climate context, and user-added observations attached to H3 cells.
...
- Preserve model version, product year, confidence layer or quality metadata where available, Sentinel feature vintage, source format, license, and ethical-use notes.
...
### GeoLibre Desktop GIS And Whitebox Sidecar

Reference: `https://github.com/opengeos/GeoLibre`

Fork: not needed for vmesh unless a later implementation task deliberately vendors or ports a narrow component.

GeoLibre is a lightweight desktop/web GIS prototype built with Tauri v2, React, TypeScript, MapLibre GL JS, DuckDB-WASM Spatial, and deck.gl. GitHub license metadata is currently missing, while the README states MIT, so license should be manually verified before code reuse. The app supports many local and remote layer formats, including GeoJSON, GeoParquet, GeoPackage, Shapefile, FlatGeobuf, KML/KMZ, GML, XYZ, WMS, vector tiles, COG/GeoTIFF, MBTiles, ArcGIS FeatureServer and VectorTileServer, PMTiles, Zarr, LiDAR, and Gaussian splats.

The useful "700 tools" sidecar is specifically the Whitebox integration, not the generic `/run` sidecar. The optional FastAPI backend exposes `/whitebox/status`, `/whitebox/tools`, `/whitebox/run`, `/whitebox/jobs/{id}`, and `/whitebox/output`, lazily bootstraps `whitebox-workflows`, normalizes tool parameters, runs jobs in background threads, and retains job state. Its remote Whitebox catalog snapshot currently lists `747` tools. The more general sidecar `/run` endpoint is still a placeholder, and GDAL/Rasterio/GeoPandas/DuckDB/Leafmap/GeoAI/SamGeo are described as future integrations rather than installed runtime capabilities.

Potential vmesh relevance:

- Good reference for a local/desktop GIS operator surface that can load many file formats while keeping the main app renderer-focused.
- Useful pattern for a sidecar tool catalog: status endpoint, typed parameter normalization, background jobs, retained output refs, and safe output reading.
- Whitebox can inform terrain/hydrology workflows such as slope, watershed, flow accumulation, drainage, raster/vector terrain analysis, and LiDAR-adjacent processing once vmesh has source-backed inputs.
- MapLibre plugin patterns for layer control, basemap control, swipe, LiDAR, GeoParquet, DuckDB, ArcGIS layers, and geo-editing are relevant to future vmesh inspection/admin tools.

Implementation boundary:

- Do not treat GeoLibre as a source resolver for USA/Canada 1 m DTM/DSM. It does not prove coverage and does not replace vmesh's official-source probe and package-worker evidence.
- Do not promote the Whitebox catalog into an unrestricted production toolbox. vmesh needs curated allowlists, per-tool input/output schemas, job budgets, memory/time limits, provenance, and live-proof artifacts.
- Keep dense LiDAR/point-cloud inspection in a sidecar or dedicated viewer; the primary vmesh map should consume derived DEM/DSM, hillshade, contours, PMTiles, GeoTIFF/COG, or H3 summaries.
- Any code reuse needs manual license verification because GitHub does not currently report a license object for the repo.
...
- Support future source/local fusion: a hex can store source-backed context plus graph-derived access and redundancy summaries after review.
...
The app includes typed provider interfaces and deterministic mock data for deferred weather/hazard/solar scaffolding, food networks, property signals, Reticulum gateway status, Meshtastic bridge status, and local LLM status. Deferred analysis scaffolding is not exposed as a current user-facing conclusion.
