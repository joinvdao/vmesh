...
import {
  ...
  sanitizeTextLabel,
  type BuildingPackageWorkerInput,
...
function publicHandoff(input: BuildingPackageWorkerInput) {
  const handoff = createBuildingPackageWorkerHandoff(input, plannerOptions());
  const publicPlan = redactPackagePlanForPublic(handoff.plan);
...
export async function POST(req: NextRequest) {
  ...
  if (contentLength > MAX_PACKAGE_PLAN_BODY_BYTES) {
    return jsonResponse({ error: "Building package request body is too large." }, 413);
  }
  if (!contentType.toLowerCase().includes("application/json")) {
    return jsonResponse({ error: "Building package requests must use application/json." }, 415);
  }

  const body = await parseBody(req);
  if (!body) {
    return jsonResponse({ error: "Invalid JSON building package request." }, 400);
  }
  ...
  if (!aoi) {
    return jsonResponse(
      {
        error:
          "A building package handoff requires an AOI with h3Id, centroid { latitude, longitude }, or bounds [west, south, east, north]."
      },
      400
    );
  }

  const preferredSourceIds = parsePreferredSourceIds(body.preferredSourceIds);
