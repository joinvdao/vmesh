...
function createRecord([label, latitude, longitude]) {
  ...
  const floodLevel = Math.min(100, Math.round(48 * 0.35 + rain * 1.2));
  const fireLevel = Math.min(100, Math.round(42 * 0.45 + heat * 0.62 + wind * 0.28 - rain * 0.18));
  const solarLevel = Math.max(
    0,
