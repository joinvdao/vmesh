...
export const MAPBOX_SATELLITE_PROVIDER_ID = "mapbox-satellite-global";
export const MAPBOX_SATELLITE_LEGACY_PROVIDER_ID = "mapbox-satellite-optional";
...
const MAPBOX_SATELLITE_PROVIDER_IDS = new Set([
  MAPBOX_SATELLITE_PROVIDER_ID,
  MAPBOX_SATELLITE_LEGACY_PROVIDER_ID
]);
...
function createConfiguredMapboxTileUrl(options: ImageryProviderRegistryOptions): string {
  if (options.mapboxProxyUrl) return options.mapboxProxyUrl;
  if (!options.mapboxToken) return "";

  try {
    return createMapboxSatellitePublicTileUrl(options.mapboxToken);
  } catch {
    return "";
  }
}
...
export function getImageryProviderRegistry(
  ...
  const mapboxTileUrl = createConfiguredMapboxTileUrl(options);
  const hasMapboxAccess = Boolean(mapboxTileUrl);
  const preferredProviderId =
    options.preferredProviderId === MAPBOX_SATELLITE_LEGACY_PROVIDER_ID
      ? MAPBOX_SATELLITE_PROVIDER_ID
    ...
    withPriority(
      {
        ...
        kind: "mapbox-satellite-global",
        sourceUrl: mapboxTileUrl,
        attribution: "Mapbox and upstream satellite imagery providers",
        license: "Mapbox terms; visual-context use only unless separately reviewed",
        requiresApiKey: true,
        status: hasMapboxAccess ? "available" : "requires-api-key",
        notes:
          options.mapboxProxyUrl === MAPBOX_SATELLITE_PROXY_TILE_URL
            ? "Global ortho-style visual imagery through the server-side Mapbox proxy. This is visual context, not terrain, parcel, legal, or survey truth."
            : "Global ortho-style visual imagery for deployments with a restricted public pk.* token. This is visual context, not terrain, parcel, legal, or survey truth."
      },
      80
    ),
    withPriority(
...
export function selectImageryProvider(
  ...
  const normalizedPreferredProviderId =
    preferredProviderId === MAPBOX_SATELLITE_LEGACY_PROVIDER_ID
      ? MAPBOX_SATELLITE_PROVIDER_ID
      : preferredProviderId;

  return (
    providers.find((provider) => provider.id === normalizedPreferredProviderId) ??
    providers.find(
...
export function toImageryRasterSource(
  ...
  return {
    type: "raster",
    tiles: [url],
    tileSize: MAPBOX_SATELLITE_PROVIDER_IDS.has(provider.id) ? 512 : 256,
    attribution: provider.attribution
  };
}
