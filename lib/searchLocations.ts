...
function offlineLocationRank(location: OfflineLocation, normalizedQuery: string): number {
...
export function getOfflineLocationSuggestions(query: string, limit = 6): SearchLocationResult[] {
  ...
  return offlineLocations
    .map((location) => ({ location, rank: offlineLocationRank(location, normalized) }))
    .filter(({ rank }) => Number.isFinite(rank))
    .sort(
      (left, right) =>
        left.rank - right.rank || left.location.name.localeCompare(right.location.name)
    )
    .slice(0, limit)
