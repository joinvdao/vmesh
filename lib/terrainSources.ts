...
export const TERRAIN_RASTER_PREVIEW_LAYER_ID = "terrain-source-raster-preview";
export const ENV_TERRAIN_PROVIDER_ID = "env-raster-dem";
export const SOURCE_AUTO_DTM_PROVIDER_ID = "source-auto-dtm-preview";
export const SOURCE_AUTO_BEST_DTM_PROVIDER_ID = "source-auto-best-dtm-preview";
export const SOURCE_AUTO_DSM_PROVIDER_ID = "source-auto-dsm-preview";
...
export function getTerrainProviderRegistry(
  ...
  const registry: TerrainProviderConfig[] = [
    ...
    withPriority(
      {
        id: SOURCE_AUTO_DTM_PROVIDER_ID,
        label: "Official DTM Source Preview",
        kind: "source-raster-preview",
        encoding: "raster-preview",
        tileSize: 256,
        maxzoom: 16,
        attribution:
          "USGS 3DEP / Natural Resources Canada HRDEM / Government of British Columbia LidarBC",
        license: "USGS public domain; Open Government Licence - Canada; BC open data terms",
        requiresApiKey: false,
        coverage:
          "USA via USGS 3DEP 1m DEM where the product index covers the AOI; Canada defaults to partial 2m HRDEM nationally, so use HRDEM 1m or provincial sources such as LidarBC only where direct source coverage is proven",
        resolution:
          "1m DTM only where official source-native coverage is proven; lower-resolution HRDEM/Mapterhorn output is not accepted for the current 1m milestone",
        status: "available",
        sourceUrl: "/api/terrain/source-preview/source-auto/dtm/{z}/{x}/{y}",
        notes:
          "Source-backed DTM inspection layer. Uses Mapterhorn attribution only as a source-family clue; worker/package output must resolve official upstream sources and fail closed on 1m gaps."
      },
      11
    ),
    withPriority(
      {
        id: SOURCE_AUTO_BEST_DTM_PROVIDER_ID,
        label: "Best Official DTM Preview",
        kind: "source-raster-preview",
        encoding: "raster-preview",
        tileSize: 256,
        maxzoom: 16,
        ...
        requiresApiKey: false,
        coverage:
          "USA via USGS 3DEP 1m DEM where covered; Canada via proven 1m HRDEM/provincial sources where available and explicit 2m HRDEM fallback where that is the best official source",
        resolution:
          "Best official DTM preview: 1m where proven, otherwise explicitly labelled 2m HRDEM for Canada; not a strict 1m milestone pass",
        status: "available",
        sourceUrl: "/api/terrain/source-preview/source-auto-best/dtm/{z}/{x}/{y}",
        notes:
          "Practical source-backed DTM inspection layer for Canada gaps. It does not satisfy strict 1m evidence when the response headers report 2m."
      },
      12
    ),
    withPriority(
      {
        id: SOURCE_AUTO_DSM_PROVIDER_ID,
        label: "Official DSM Source Preview",
        kind: "source-raster-preview",
        encoding: "raster-preview",
        tileSize: 256,
        maxzoom: 16,
        attribution:
          "USGS 3DEP LPC / Natural Resources Canada HRDEM / Government of British Columbia LidarBC",
        license:
          "USGS public domain and source project notices; Open Government Licence - Canada; BC open data terms",
        requiresApiKey: false,
        coverage:
          "USA via USGS 3DEP Lidar Point Cloud DSM worker where LPC coverage is proven; Canada via proven HRDEM/LidarBC DSM sources where available",
        resolution:
          "1m surface DSM preview where official source-native DSM or point-cloud coverage is proven; DSM is surface context and must not be promoted to bare-earth DTM",
        status: "available",
        sourceUrl: "/api/terrain/source-preview/source-auto/dsm/{z}/{x}/{y}",
        notes:
          "Worker-rendered surface-model inspection layer. USA tiles can invoke bounded LPC point-cloud derivation; Canada tiles use role-specific HRDEM/LidarBC DSM source paths where proven."
      },
      13
    ),
    withPriority(
...
export function getTerrainProviderCandidates(
  ...
  priorityIds.forEach((id) => {
    ...
    if (provider && isMapReadyTerrainProvider(provider) && !ordered.includes(provider)) {
      ordered.push(provider);
    }
  });

  providers.forEach((provider) => {
    if (isMapReadyTerrainProvider(provider) && !ordered.includes(provider)) {
      ordered.push(provider);
    }
  });
...
export function toRasterPreviewSource(
  provider: TerrainProviderConfig
): RasterSourceSpecification | null {
  if (provider.kind !== "source-raster-preview") return null;
...
export function isMapReadyTerrainProvider(provider: TerrainProviderConfig): boolean {
  return toRasterDemSource(provider) !== null || toRasterPreviewSource(provider) !== null;
}
