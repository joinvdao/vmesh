...
| Infrastructure context data       | Sensitive                    | Requires provenance and access rules.                               |
...
Solar, wind, and sector-map providers follow the same selected-location privacy rule. Direct browser calls for irradiance, cloud cover, wind, or forecast data may expose the selected H3 centroid or AOI to a provider. Privacy-sensitive deployments should use brokered source refs, local hub caches, precomputed H3 summaries, or offline source manifests.
...
The macro-atlas source broker records rejected-source reasons and STAC/source manifests without downloading or storing raw provider artifacts by default. Source manifests are safe to commit only when they contain generic fixtures, public provider metadata, no private AOIs, no exact private addresses, no tokens, and no downloaded rasters/tiles. Real payloads, downstream worker outputs, and local hub caches stay outside Git.

## Ecosystem Source Broker Rules

The ecosystem source broker may expose source plans, source probes, STAC/source manifests, typed ecosystem manifests, provider IDs, provider-native ref kinds, optional artifact kinds, and cache keys. It must not expose:
...
Source plans are metadata only. Heavy package generation must run in a downstream, local/server, or explicitly configured worker that preserves source license, attribution, CRS, vertical datum where relevant, acquisition/processing time, confidence, and limitations before any artifact becomes app-ready. Heavy GIS or ecosystem-data storage is not the vmesh default.
...
- Three.js orbit-globe textures, including the bundled NASA Blue Marble raster, are visual context only. They may guide the atlas experience, but production claims must come from MapLibre/provider registries, STAC/source manifests, H3 summaries, and explicit provenance.
...
- Treat Mapterhorn as a first-class open terrain fallback provider through PMTiles and catalog metadata, with attribution and upstream dataset notices preserved.
- Treat official source-preview routes separately from fallback terrain. USA source-preview uses USGS 3DEP 1 m DTM where coverage is proven. Mapterhorn's Canada terrain entry is Natural Resources Canada HRDEM `cahrdem2`, partial, 2 m, so it cannot support a Canada-wide 1 m claim. Canada source-preview may claim 1 m only where direct official HRDEM 1 m or provincial sources such as LidarBC prove role-specific source pixels; otherwise it must fail closed or label lower-resolution coverage honestly. Do not relabel fallback tiles as source truth.
...
- Mapbox Satellite is optional and token-gated for both base-globe and global ortho-style imagery use. Secret-class Mapbox tokens must stay in server-only `MAPBOX_TOKEN` and be accessed through the local proxy route; only restricted public `pk.*` tokens may use `NEXT_PUBLIC_MAPBOX_TOKEN`. Do not commit screenshots with private tokens or token-bearing tile URLs.
