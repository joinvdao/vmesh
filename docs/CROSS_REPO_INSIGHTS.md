...
The goal is to share durable patterns: source registries, STAC/source manifests, provider research, interface lessons, provenance rules, evaluation fixtures, and implementation constraints. Downstream apps should be able to request data availability from vmesh and consume clean provider-native refs/manifests instead of hard-coding provider branches.
...
- source registry;
- source probes;
- ecosystem STAC/source broker;
- package planner for optional worker/cache modes;
- PMTiles/vector/raster package contracts for explicit derived outputs;
- cache/index contracts for deployments that opt into storage;
- provider metadata;
- attribution and provenance;
- simple API and future MCP-facing tool surface.

Downstream apps remain free to build their own UI, simulation, reporting, advisory, editing, GIS/ecosystem worker, and storage experiences on top of these source-broker responses.
...
## Ecosystem Broker Loop

vmesh should develop high-fidelity STAC/source contracts for terrain role, CRS, vertical datum, imagery, buildings, roads, parcels, hydrology, vegetation, soils, biodiversity, climate, weather, infrastructure, food-system/local assets, provenance, and fidelity ladders.

Downstream apps should consume the resulting STAC Items, typed ecosystem records, provider-native refs, and H3/source summaries without knowing whether the data came from Mapterhorn, Mapzen, OSM, Overture, Sentinel, a regional DTM, ArcGIS FeatureServer, object storage, climate/weather APIs, soil grids, biodiversity registries, food-system datasets, or a local cache.

vmesh is both a standalone atlas and an ecosystem source aggregator. The public contract should be generic: a downstream app sends a coordinate/AOI/H3/place request, vmesh returns a STAC-compatible spatial response plus typed ecosystem records with provider-native asset refs, provenance, rejected-source reasons, and optional cache/job metadata only when that deployment enables it. The public vmesh repo should not identify private consumer repos, exact user locations, local paths, provider credentials, paid quotes, or unpublished roadmaps.

The current cross-app default is source brokering for a boundary-first property treatment workflow:

```text
selected H3 / AOI / property boundary
  -> vmesh STAC/source response
  -> downstream app GIS/ecosystem worker fetches/processes/stores when needed
  -> optional PMTiles / COG / GeoParquet / H3 summaries / map plates / manifests in the downstream app
```
...
cues. vmesh can define the schema and broker refs for these as
`scene-annotations.json`, H3-attached `visual-observation` records, or
GeoJSON/PMTiles overlays when georegistration is available; downstream apps or
explicit cache deployments decide where they are stored. These annotations help
prompt preparation, report notes, generated world recognisability, and QA, but
they do not become measured geometry or legal truth without reviewed geospatial
anchoring.

Public cached PMTiles are a good optional delivery primitive for open/generalized layers, but they are not the default vmesh contract. Private property packages, paid provider outputs, exact user boundaries, report assets, and generated downstream outputs belong in the downstream app, owner-private object refs, or local hub storage unless a vmesh deployment explicitly opts into secured storage. Auth must protect the data URL itself, not only the browser UI.
...
The source-broker discipline now becomes the generic vmesh downstream rule:
...
- Treat STAC/source manifests and typed ecosystem records as the default contracts for terrain, imagery, landcover, environment, soils, hydrology, biodiversity, local assets, contour, H3 summary, and provenance payloads.
- Keep the browser as a renderer of map-ready outputs and H3 summaries, while heavy Overture/OSM/Sentinel/SEN2SR/climate-grid processing happens in downstream, local-hub, or explicitly configured server workers.

This pattern lets any downstream app consume sanitized H3/source summaries, ecosystem records, and provider-native refs without copying private AOIs, provider tokens, local planning context, or downloaded payloads into vmesh.
