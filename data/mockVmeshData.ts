...
export const mockMacroProvenance: MacroProvenance = {
  ...
  confidence: 74,
  limitations:
    "Mock macro data for UI and pipeline development. It is not operational climate, fire, flood, or solar intelligence.",
  license: "Fictional sample data; no paid APIs or scraped sources"
};

function macroRiskClass(value: number): "low" | "moderate" | "high" | "severe" {
...
export function createMockMacroCellSummary(record: VmeshHexRecord): MacroCellSummary {
  ...
  const floodLevel = Math.min(100, Math.round(24 + ((seed * 10) % 52) + rain * 0.8));
  const fireLevel = Math.min(100, Math.round(18 + heat * 0.62 + wind * 0.28 - rain * 0.18));
  const solarLevel = Math.max(0, Math.min(100, Math.round(40 + (100 - cloud) * 0.38 + heat * 0.12)));
