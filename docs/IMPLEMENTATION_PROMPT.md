...
Build the first MVP slice for `vmesh`, an atlas of antifragility built around an H3 mesh overlay. The experience should show a polished 3D geospatial dashboard where each hex can hold macro data such as climate, water, energy, biodiversity, infrastructure, and risk, plus micro data such as properties for sale, farmers markets, farms, local producers, community assets, and user-added records.

Use `docs/DESIGN_DIRECTION.md` as the visual target. The first screen should resemble a light, operational geospatial cockpit: left navigation, top search/header controls, globe-first H3 mesh map, floating map tools, macro-to-micro layer control, right selected-hex panel, bottom analytics strip, and footer telemetry.
...
- `components/Panels/BottomAnalytics.tsx`
...
- Each hex should include macro pillar scores, micro asset counts, source/provenance fields, confidence, and a derived antifragility/resilience score.
...
- Bottom dashboard: `h-72`, absolute bottom above footer, horizontally scrollable analytics cards.
...
- `extruded: true`.
- `getElevation: d => d.antifragilityScore * 10`.
- `pickable: true`.
...
- Main score, status badge, and source confidence.
...
- Recharts sparkline for a 5-year trend.

`BottomAnalytics` must include:

- Climate trend LineChart from 1990 to 2040.
- Energy radial progress card showing `74% High`.
...
- Do not introduce hidden provider spend, secret exposure, analytics PII, or real scraped listing data.
- Keep user-added data local/mock unless persistence is explicitly approved.
- Update docs when product, system, privacy, operations, or analytics behavior changes.
