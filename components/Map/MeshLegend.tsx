...
export function MeshLegend() {
  ...
  return (
    <div className="absolute bottom-6 left-6 z-20 w-[280px] rounded-[10px] border border-[#B6D9D1] bg-white/90 p-4 shadow-[0_14px_35px_rgba(31,53,58,0.12)] backdrop-blur">
      <div className="flex items-center gap-2 text-sm font-medium text-[#2D545B]">
        {activeLayers.macro
          ? `${MACRO_LAYER_LABELS[selectedMacroLayer]} Layer`
          : "Antifragility Index"}
        <CircleHelp className="h-3.5 w-3.5 text-[#6F8589]" />
      </div>
      <div
      ...
      />
      <div className="mt-2 flex justify-between text-[11px] text-[#6F8589]">
        <span>0</span>
        <span>25</span>
        <span>50</span>
        <span>75</span>
      ...
      <div className="mt-3 flex justify-between text-xs text-[#5F777C]">
