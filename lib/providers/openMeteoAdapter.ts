...
function riskClass(score: number): "low" | "moderate" | "high" | "severe" {
...
export function createMacroSummaryFromOpenMeteo({
  ...
  const heatStress = clamp((maxTemp - 24) * 4 + (averageHumidity - 55) * 0.25);
  const windStress = clamp(maxGust * 1.2);
  const rainStress = clamp(rain72h * 2.2);
  const droughtProxy = clamp(70 - rain72h * 4 - averageHumidity * 0.35);
  const weatherStress = Math.round(
    clamp(heatStress * 0.32 + windStress * 0.22 + rainStress * 0.24 + droughtProxy * 0.22)
  );
  const fireScore = Math.round(
    clamp(heatStress * 0.34 + droughtProxy * 0.38 + windStress * 0.22 + (100 - averageCloud) * 0.06)
  );
  const floodScore = Math.round(clamp(rainStress * 0.55 + rain72h * 1.1 + windStress * 0.1));
  const solarScore = Math.round(clamp(100 - averageCloud * 0.72 + Math.max(0, maxTemp - 18) * 0.6));
