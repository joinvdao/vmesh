...
Your task is to build the next vmesh macro package pipeline: a source-backed, versioned, testable data package path that turns reviewed macro providers into H3 summaries, while strengthening browser visual verification of the globe, close-zoom transition, and the distinction between decorative globe texture and source-backed map output.
...
vmesh is an atlas of antifragility. Macro layers must help users reason about climate, water, energy, biodiversity, terrain, hazard, vegetation, and infrastructure conditions at H3 scale.
...
- Versioned local/server package manifests for broader macro layers.
