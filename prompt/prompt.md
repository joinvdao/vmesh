# vmesh Next Phase: Source Atlas And Open Geospatial Source Funnel

You are Codex acting as a principal geospatial systems engineer, frontend architect, and cartographic design lead.

Repo: vmesh, "Source Atlas".

Objective: elevate vmesh beyond a simple MVP into a beautiful source-honest geospatial atlas. Do not start from scratch. Preserve the existing Next.js App Router, React, TypeScript, Zustand, MapLibre, deck.gl, H3, PMTiles, Tailwind, docs workflow, and current globe-first visual direction.

This phase focuses on source-backed geospatial data:
...
- open map/vector context such as roads, buildings, water, parcels, land use, and source provenance.

Climate, hazard, solar, wind, sector-map, and weather-derived analysis outputs are deferred from the visible product. Do not add those as user-facing layer families in this phase.

The product is a source-data aggregator first. Do not add recommendations, automated conclusions, or authoritative analysis yet. Show data, source, status, freshness, and limitations clearly.
...
- Every future climate record must include provider, variable names, run time or observed time, forecast horizon where relevant, license, and limitations, and must stay out of the visible workflow until deliberately reintroduced.
...
This phase should show context and provenance only. Do not add final place analysis or recommendations.
...
- deferred analysis provider boundaries;
